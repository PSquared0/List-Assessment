...
def repeat_string(name, number):
    """prints string as many times as indicated by interger"""
    for i in int(range(number)):
        print 'name'


def  print_sign(n):
    """indicates if an number is qeual to, higher or lower than zero"""
...
def is_divisible_by_three(n):
    if n % 3 == 0:
        """ Indicates if a number is divisable by 3"""

        print True
    else: 
        print False

def num_spaces(spaces):
    for i in spaces.split(" "):
        print spaces.count(i)

def total_meal_price(price,tip):
    """ calucaltes tip on price of meal, defaults to 15 percent with no tip inidcated"""
